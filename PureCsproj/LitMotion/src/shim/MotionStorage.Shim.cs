...
namespace LitMotion
{
    ...
    // CLI：去掉 AnimationCurve.Evaluate / UnsafeUtility.As / NativeArray 原生分配，避免 JIT ECall
        ...
        where TAdapter : unmanaged, IMotionAdapter<TValue, TOptions>
    {
        ...
        public MotionHandle Create(ref MotionBuilder<TValue, TOptions, TAdapter> builder)
        {
            ...
            {
                throw new NotSupportedException("CLI 不支持 Ease.CustomAnimationCurve");
            }
            ...
            if (buffer.ImmediateBind && buffer.UpdateAction != null)
            {
                managedDataRef.UpdateUnsafe(
                    default(TAdapter).Evaluate(
                        ...
                        new()
                        {
                            Progress = EaseUtility.Evaluate(0f, parameters.Ease),
                            Time = state.Time,
                        }
                ));
            }
        ...
        {
            var list = denseIndexList.Length == 0 ? Array.Empty<SparseIndex>() : new SparseIndex[denseIndexList.Length];
        ...
        {
            ref var slot = ref GetSlotWithVarify(handle, checkIsInSequence);
            return ref MemoryMarshal.Cast<MotionData<TValue, TOptions>, MotionData>(
                MemoryMarshal.CreateSpan(ref unmanagedDataArray[slot.DenseIndex], 1))[0];
        }
