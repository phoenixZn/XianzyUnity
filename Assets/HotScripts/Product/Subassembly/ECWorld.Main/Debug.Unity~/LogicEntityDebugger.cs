...
public partial class LogicEntityDebugger : MonoBehaviour, IVarEnvFriend
{
    ...
    void Start()
    {
        ...
        var curEnvState = G.EnvStates?.CurrentState as EnvBattleState;
        if (curEnvState == null)
        {
            KLogger.LogError("LogicEntityDebugger curEnvState == null");
            return;
        }

        TryAutoLinkFromGameObject(curEnvState.Worlds?.LogicWorld);
    }
    ...
    public void Link(LogicEntity entity)
    {
        if (_entity != null)
        {
            throw new Exception($"LogicEntityDebugger is already linked to {_entity.GetLogStr()}");
        }
        _entity = entity;
    ...
    void TryAutoLinkFromGameObject(LogicWorld logicWorld)
    {
        ...
        var entity = logicWorld.GetEntitiesWithComUnityObjectRelated(gameObject.GetInstanceID());
        if (entity == null)
        {
