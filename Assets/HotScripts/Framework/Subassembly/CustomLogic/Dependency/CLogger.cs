...
public class LogFileWriter : ILogWriter
{
    ...
    public string GetLogDir()
    {
        switch (m_logPlatform)
        {
            case LogPlatform.Android:
                return Application.persistentDataPath + PATH_END + "/" + CLogger.sm_product_prefix + PATH_END;
            case LogPlatform.IOS:
                return Application.temporaryCachePath + "/" + CLogger.sm_product_prefix + PATH_END;
            case LogPlatform.Win:
                return Application.dataPath + "/../" + CLogger.sm_product_prefix + PATH_END;
    ...
    public void Init()
    {
        ...
        {
#if CONSOLE_CLIENT
            if (!Directory.Exists(CLogger.LOG_DIR))
            {
                Directory.CreateDirectory(CLogger.LOG_DIR);
            }
            string fileName = CLogger.LOG_DIR + string.Format("{0}_{1:D2}_{2:D2}_{3:D2}_{4:D2}_{5:D2}.{6:D5}.{7}",
